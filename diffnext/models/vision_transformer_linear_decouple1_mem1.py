...
class LinearAttentionDecoder(nn.Module):
    ...
    def forward(self, x) -> torch.Tensor:
        ...
        with torch.cuda.amp.autocast(enabled=not use_fp32_attention):
            ...
            kv = self.compute_grouped_memory(k, v, M=4)
    ...
    def compute_grouped_memory(self, k, v, M=4):
        """
        Args:
        ...
        Returns:
            kv_combined: shape (B, num_heads, head_dim, head_dim)
        """
        B, H, N, D = k.shape
        assert N % M == 0, "N must be divisible by M"
        group_size = N // M

        # reshape to (B, H, M, group_size, D)
        k_grouped = k.view(B, H, M, group_size, D)
        v_grouped = v.view(B, H, M, group_size, D)

        # transpose k_grouped to get (B, H, M, D, group_size)
        kT_grouped = k_grouped.transpose(-2, -1)

        # compute memory for each group: (B, H, M, D, D)
        memory_per_group = torch.matmul(kT_grouped, v_grouped) * (group_size ** -1)

        # gating weights: (B, H, M, 1, 1)
        gate_weights = torch.softmax(self.gate_logits, dim=2)

        # apply gating: weighted sum across M groups → (B, H, D, D)
        kv_combined = (memory_per_group * gate_weights).sum(dim=2)

        return kv_combined  # shape (B, H, D, D)
