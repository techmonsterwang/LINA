...
class Attention(nn.Module):
    ...
    def __init__(self, dim, num_heads, qkv_bias=True, proj_drop=0., kernel_function=nn.ReLU, fp32_attention=True):
        ...
        self.output_gate = nn.Linear(dim, dim, bias=qkv_bias)
    ...
    def forward(self, x) -> torch.Tensor:
        ...
        with torch.cuda.amp.autocast(enabled=not use_fp32_attention):
            ...
            output = q @ kv * z   # (B, H, N, C/H) @ (B, H, C/H, C/H) * (B, H, N, 1) = (B, H, N, C/H): (32, 6, 256, 64) 
            output = output.transpose(1, 2).flatten(2)    # (B, N, C): (32, 256, 384)  
            
        output = output.to(dtype)

        output = self.rms_norm(output)        
        output = torch.sigmoid(self.output_gate(x)) * output                       
