...
class Transformer3DModel(nn.Module):
    ...
    def diff_annealing_steps(self, curr_iter: int, ar_num_iter: int = 64, strategy: str = "linear", min_schedule: int = 5) -> int:
        upper_iter = ar_num_iter - 1
        max_schedule = 25
        span = max_schedule - min_schedule

        if strategy == "linear":
            ratio = (upper_iter - curr_iter) / upper_iter
        elif strategy == "cosine":
            ratio = (math.cos(math.pi * curr_iter / upper_iter) + 1) / 2
        elif strategy == "constant":
            return min_schedule
        elif strategy == "two-stage":
            return max_schedule if curr_iter < upper_iter / 2 else min_schedule
        else:
            raise ValueError(f"Unknown sampling strategy: {strategy}")

        diff_steps = int(ratio * span + min_schedule)
        return diff_steps
    ...
    def generate_frame(self, states: Dict, inputs: Dict):
        ...
        for i, num_preds in enumerate(self.progress_bar(all_num_preds, inputs.get("tqdm2", False))):
            ...
            # self.sample_scheduler.set_timesteps(diff_annealing_steps(curr_iter=i, ar_num_iter=len(all_num_preds), strategy=inputs.get("anneal_strategy", "myown"), min_schedule=5))
            self.sample_scheduler.set_timesteps(self.diff_annealing_steps(curr_iter=i, ar_num_iter=len(all_num_preds), strategy='linear', min_schedule=5))
    ...
    def train_video(self, inputs):
        ...
        c, pos = self.video_pos_embed(torch.cat([bov, c], dim=1)), None # c: [64, 1, 64, 768], pos: None
        if self.image_pos_embed:
        ...
        c = self.video_encoder(c.flatten(1, 2), inputs["c"], pos=pos) # c: [64, 64, 768]
        ...
        if self.image_pos_embed:
            pos = self.image_pos_embed.get_pos(1, bs, self.image_encoder.patch_embed.hw) # pos: [64, 256, 3]
        # z: [64, 256, 768]
