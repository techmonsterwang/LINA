...
class LinearAttentionDecoder(nn.Module):
    ...
    def __init__(self, dim, num_heads, qkv_bias=True, proj_drop=0., kernel_function=nn.ReLU, fp32_attention=True):
        ...
        self.key_scale_gamma = nn.Parameter(torch.ones(1, num_heads, 320, 1))  # shape: (1, H, N, 1)
        self.key_scale_tau = nn.Parameter(torch.ones(1, num_heads, 320, 1))  # shape: (1, H, N, 1)
        self.value_scale = nn.Parameter(torch.ones(1, num_heads, 320, 1))  # shape: (1, H, N, 1)
    ...
    def forward(self, x) -> torch.Tensor:
        ...
        k_gamma = k * self.key_scale_gamma  # Apply key scaling
        k_tau = k * self.key_scale_tau  # Apply key scaling
        v = v * self.value_scale  # Apply value scaling

        with torch.cuda.amp.autocast(enabled=not use_fp32_attention):
            z = 1 / (q @ k_tau.mean(dim=-2, keepdim=True).transpose(-2, -1) + 1e-6)
            kv = (k_gamma.transpose(-2, -1) * (N ** -0.5)) @ (v * (N ** -0.5))
            attn_out = q @ kv * z
        ...
        # dwc branch
        # ==============================
        x_tail = x[:, 64:, :].reshape(B, 16, 16, -1).permute(0, 3, 1, 2)    # shape: (B, C, 16, 16)
        conv_out = self.dw_conv(x_tail)    # shape: (B, C, 16, 16)
