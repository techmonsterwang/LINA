...
class Attention(nn.Module):
    ...
    def forward(self, x, HW=None):
        ...
        kv = self.kv(x).reshape(B, N, 2, C).permute(2, 0, 1, 3)   # (2, B, N, C)
        k, v = kv[0], kv[1]   # (B, N, C)

        q = self.kernel_function(q) + 1e-6   # (B, N, C)
        k = self.kernel_function(k) + 1e-6   # (B, N, C)

        q = q.reshape(B, N, self.num_heads, -1).permute(0, 2, 1, 3).to(dtype)   # (B, H, N, C/H)
        k = k.reshape(B, N, self.num_heads, -1).permute(0, 2, 1, 3).to(dtype)   # (B, H, N, C/H)
        v = v.reshape(B, N, self.num_heads, -1).permute(0, 2, 1, 3).to(dtype)   # (B, H, N, C/H)
