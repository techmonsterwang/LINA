...
class LinearAttentionDecoder(nn.Module):
    ...
    def __init__(self, dim, num_heads, qkv_bias=True, proj_drop=0., kernel_func='relu', fp32_attention=True):
        ...
        self.residual = nn.Sequential(
            nn.Conv1d(dim, dim, kernel_size=1, groups=num_heads),
            nn.GELU(),
            nn.Conv1d(dim, dim * 25, kernel_size=1, groups=num_heads)
        )
    ...
    def forward(self, x) -> torch.Tensor:
        ...
        with torch.cuda.amp.autocast(enabled=not use_fp32_attention):
            ...
            res_weight = self.residual(x.mean(dim=1).unsqueeze(dim=-1)).reshape(b * c, 1, 5, 5)  # e.g., 3 = kernel_size, torch.bfloat16
        ...
        conv_out = F.conv2d(x_tail, res_weight, None, padding=(2, 2), groups=b * c)  # shape: (1, B*C, 16, 16), torch.bfloat16
