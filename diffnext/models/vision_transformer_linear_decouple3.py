...
class LinearAttentionEncoder(nn.Module):
    ...
    def __init__(self, dim, num_heads, qkv_bias=True, proj_drop=0., kernel_func='relu', fp32_attention=True):
        ...
        self.residual = nn.Sequential(
            ...
            nn.GELU(),
            nn.Conv1d(dim, dim * 3, kernel_size=1, groups=num_heads)
        )
    ...
    def forward(self, x) -> torch.Tensor:
        ...
        with torch.cuda.amp.autocast(enabled=not use_fp32_attention):
            ...
            res_weight = self.residual(x.mean(dim=1).unsqueeze(dim=-1)).reshape(b * c, 1, 3)  # e.g., 3 = kernel_size
            ...
            residual = F.conv1d(v, res_weight, bias=None, padding=1, groups=b * c)  # padding = (kernel_size // 2)
