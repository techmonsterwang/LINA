...
class Attention(nn.Module):
    ...
    def __init__(self, dim, num_heads, qkv_bias=True, attn_drop=0., proj_drop=0., mediator_num=64, fp32_attention=True):
        ...
        self.pool = nn.AdaptiveAvgPool1d(mediator_num)
    ...
    def forward(self, x) -> torch.Tensor:
        ...
        # q, k, v: B, N, C
        mediator_tokens = self.pool(q.permute(0, 2, 1)).permute(0, 2, 1)  # -> [B, mediator_num, C]
        q = q.reshape(B, N, num_heads, head_dim).permute(0, 2, 1, 3)  # -> [B, h, N, c]
        k = k.reshape(B, N, num_heads, head_dim).permute(0, 2, 1, 3)  # -> [B, h, N, c]
        v = v.reshape(B, N, num_heads, head_dim).permute(0, 2, 1, 3)  # -> [B, h, N, c]
        mediator_tokens = mediator_tokens.reshape(B, self.mediator_num, num_heads, head_dim).permute(0, 2, 1, 3)  # -> [B, h, mediator_num, c]
        
        dtype = q.dtype

        use_fp32_attention = getattr(self, 'fp32_attention', False)     # necessary for NAN loss
        if use_fp32_attention:
            q, k, v, mediator_tokens = q.float(), k.float(), v.float(), mediator_tokens.float()

        with torch.cuda.amp.autocast(enabled=not use_fp32_attention):
            mediator_attn = self.softmax((mediator_tokens * self.scale) @ k.transpose(-2, -1))  # -> [B, h, mediator_num, N]
            mediator_attn = self.attn_drop(mediator_attn)
            mediator_v = mediator_attn @ v  # -> [B, h, mediator_num, c]

            q_attn = self.softmax((q * self.scale) @ mediator_tokens.transpose(-2, -1))  # -> [B, h, N, mediator_num]
            q_attn = self.attn_drop(q_attn)
            x = q_attn @ mediator_v  # -> [B, h, N, c]

            x = x.transpose(1, 2).flatten(2)  # -> [B, N, c]  
