...
class LinearAttentionDecoder(nn.Module):
    ...
    def __init__(self, dim, num_heads, qkv_bias=True, proj_drop=0., kernel_function=nn.ReLU, fp32_attention=True):
        ...
        # self.gate_logits = nn.Parameter(torch.zeros(1, self.num_heads, 4, 1, 1))  # For M=4 groups

    def forward(self, x) -> torch.Tensor:
        B, N, C = x.shape
        assert N == 320, f"Expected sequence length N=320, but got {N}"
        ...
        with torch.cuda.amp.autocast(enabled=not use_fp32_attention):
            ...
            attn_out = self.compute_grouped_memory_and_query(q, k, v, M=4)
        ...
        # dwc branch
        # ==============================
        x_tail = x[:, 64:, :].reshape(B, 16, 16, -1).permute(0, 3, 1, 2)
        conv_out = self.dw_conv(x_tail)
        conv_out = conv_out.reshape(B, C, -1).permute(0, 2, 1)
    ...
    def compute_grouped_memory_and_query(self, q, k, v, M=4):
        """
        Args:
            q: shape (B, num_heads, N, head_dim)
            k, v: shape (B, num_heads, N, head_dim)
            M: number of groups

        Returns:
            result: shape (B, num_heads, N, D) - final result after combining query and updated memory
        """
        B, H, N, D = k.shape
        assert N % M == 0, "N must be divisible by M"
        group_size = N // M

        # Step 1: Reshape k, v, and q into M groups
        k_grouped = k.view(B, H, M, group_size, D)   # [B, H, M, N/M, D]
        v_grouped = v.view(B, H, M, group_size, D)   # [B, H, M, N/M, D]
        q_grouped = q.view(B, H, M, group_size, D)   # [B, H, M, N/M, D]

        # Step 2: Compute memory for each group (same as before)
        kT_grouped = k_grouped.transpose(-2, -1)   # [B, H, M, D, N/M]
        memory_per_group = torch.matmul(kT_grouped, v_grouped)   # [B, H, M, D, D]

        # Step 3: Compute similarity between memory groups (using batch matrix multiplication)
        memory_similarity = torch.matmul(memory_per_group.view(B, H, M, -1), memory_per_group.view(B, H, M, -1).transpose(-1, -2))   # [B, H, M, M]

        memory_similarity = memory_similarity * (group_size ** -1)
        # Step 4: Compute attention-like weights for each memory, similar to self-attention
        # alpha_weights = torch.softmax(memory_similarity, dim=-1)   # [B, H, M, M]

        # Step 5: Use alpha weights to combine the memories (replace einsum)
        kv = torch.matmul(memory_similarity, memory_per_group.view(B, H, M, -1))   # [B, H, M, DD]

        # Step 6: Compute the result for each group (q @ kv) (replace einsum)
        result_per_group = torch.matmul(q_grouped, kv.view(B, H, M, D, D))   # [B, H, M, N/M, D]

        # Step 7: Sum the results from all groups to get the final result
        result = result_per_group.view(B, H, N, D)  # reshaping the result to (B, H, N, D)
