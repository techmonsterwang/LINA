...
class Attention(nn.Module):
    """Multi-head linear attention that can dump attention map + q/k/v."""
    ...
    def __init__(
        ...
        *,                            # -------- 新增参数区 --------
        save_attn: bool = True,      # 保存 attn map  (B,H,N,N)
        save_qkv: bool = True,       # 保存 q,k,v     (B,H,N,C/H)
        save_dir: str = "/share/project/wangjiahao/LAR/visualization/linear_attention6/",         # 统一目录
        file_prefix: str = "attn"     # 前缀，最终文件名如 attn_map.npz
    ):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        q, k, v = self.qkv(x).view(qkv_shape).permute(2, 0, 3, 1, 4).unbind(0)
        if self.pe_func:
        ...
        with torch.cuda.amp.autocast(enabled=not self.fp32_attention):
            z  = 1.0 / (q @ k.mean(dim=-2, keepdim=True).transpose(-2, -1) + 1e-6)  # (B,H,N,1)
            kv = (k.transpose(-2, -1) * (N ** -0.5)) @ (v * (N ** -0.5))            # (B,H,C/H,C/H)
            out = (q @ kv) * z                                                      # (B,H,N,C/H)
            out = out.transpose(1, 2).flatten(2)                                    # (B,N,C)

        # ---------------- dump section -----------------
        with torch.no_grad():
            if self.save_attn:
                num = q @ k.transpose(-2, -1)                           # (B,H,N,N)
                den = num.sum(-1, keepdim=True) + 1e-6
                attn = (num / den).cpu().float().numpy()
                np.savez(self.save_dir / f"{self.file_prefix}_map.npz",
                         attention=attn)

            if self.save_qkv:
                np.savez(self.save_dir / f"{self.file_prefix}_q.npz",
                         q=q.cpu().float().numpy())
                np.savez(self.save_dir / f"{self.file_prefix}_k.npz",
                         k=k.cpu().float().numpy())
                np.savez(self.save_dir / f"{self.file_prefix}_v.npz",
                         v=v.cpu().float().numpy())

        # ------------------------------------------------
        out = self.proj(out.to(orig_dtype))
